# EVProfile — Référence technique par firmware SWI / Technical Reference by SWI Firmware
> Project reference for supported runtime interfaces and observed vehicle behaviour.
...
- Sur les firmwares pris en charge, `setFcwState(2)` est associé à `setFcwAutoBrakeMode(2)`.
...
- On supported firmware, `setFcwState(2)` is paired with `setFcwAutoBrakeMode(2)`.
